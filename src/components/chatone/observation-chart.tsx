...
export function ObservationChart({ observation }: { observation: ObservationSeries }) {
  ...
  return (
    <div className="chart-shell">
      <svg
        ...
        viewBox="0 0 568 230"
        role="group"
        aria-label={`${observation.label} chart. Focus a measurement for its date and value.`}
      >
        {[0, 1, 2, 3].map((index) => (
          <g key={index}>
            <line
        ...
        {points.map((item, index) => (
          <g key={index}>
            ...
            <g
              ...
              tabIndex={0}
              role="button"
              aria-label={`${observation.dates?.[index] ?? `Measurement ${index + 1}`}: ${item.value} ${observation.unit}`}
        ...
        {point && (
          <g className="chart-tooltip" pointerEvents="none">
            ...
            <text x={tooltipX + 12} y="18" className="chart-tooltip-date">
              {observation.dates?.[active!] ?? `Measurement ${active! + 1}`}
            </text>
