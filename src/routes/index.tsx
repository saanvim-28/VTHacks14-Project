...
export const Route = createFileRoute("/")({
  head: () => ({
    meta: [
      ...
      {
        property: "og:title",
        content: "Patient Records — ChatOne",
      },
      {
...
function Index() {
  ...
  /**
   * Search the unified OpenEMR patient model.
   */
  const visiblePatients = queue.filter((patient: OpenEMRPatient) => {
    ...
    return searchableValues.join(" ").toLowerCase().includes(normalizedSearch);
  });
 ...
 *
 * This component now uses the unified OpenEMRPatient
 * model instead of the old frontend-only Patient type.
 */
